...
const MOCK = {
  ...
  dailyTarget: 80,
  breakdown: {
    body: 12, mind: 15, learning: 8, communication: 4, technical: 3, social: 0, discipline: 0,
  },
};
  ...
  return (
    <Card className="relative overflow-hidden">
      ...
      <div className="flex items-end gap-3">
        <span className="text-4xl font-extrabold tabular-nums text-white">{todayXP}</span>
        <span className="mb-1 text-sm text-white/30">/ {dailyTarget} XP</span>
      </div>
      ...
      <div className="mt-4 grid grid-cols-7 gap-1">
        {Object.entries(breakdown).map(([cat, xp]) => (
          <div key={cat} className="flex flex-col items-center gap-1">
            <div className="h-12 w-full rounded-lg bg-white/5 relative overflow-hidden">
              <div
                className="absolute bottom-0 w-full rounded-lg transition-all duration-700"
                style={{
                  height: `${Math.min(100, (xp / 20) * 100)}%`,
                  background: CATEGORY_COLORS[cat],
                  opacity: 0.8,
                }}
              />
            </div>
            <span className="text-[9px] text-white/30">{CATEGORY_LABELS[cat]}</span>
          </div>
        ))}
      </div>
    </Card>
