  ...
  return (
    <aside className="hidden md:flex md:w-56 md:flex-col md:fixed md:inset-y-0 md:pt-14">
      <div className="flex flex-1 flex-col overflow-y-auto border-r px-3 py-4"
        style={{ background: 'var(--sidebar-bg)', borderColor: 'var(--color-border)' }}>
        <div className="mb-6 px-3">
          <h1 className="text-2xl font-extrabold text-gradient">Arc</h1>
          <p className="mt-0.5 text-[11px] font-medium t-faint">Growth Tracker</p>
        </div>
        ...
        <nav className="flex-1 space-y-0.5">
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium transition-all ${
                    isActive ? 'bg-purple-500/15 text-purple-400 shadow-sm' : 't-tertiary'
                  }`
                }
              >
          ...
          <button
            ...
            className="flex w-full items-center justify-between rounded-xl px-3 py-2 text-sm font-medium t-tertiary transition-all"
          >
            <span className="flex items-center gap-3">
              <Icons.Layers className="h-4 w-4" />
              Tracking
            </span>
            <ChevronDown className={`h-4 w-4 transition-transform duration-200 ${trackingOpen ? 'rotate-180' : ''}`} />
          </button>
          {trackingOpen && (
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `flex items-center gap-3 rounded-xl px-3 py-1.5 text-sm font-medium transition-all ${
                        isActive ? 'bg-purple-500/15 text-purple-400' : 't-muted'
                      }`
                    }
                  >
        ...
        <div className="mt-auto border-t pt-2" style={{ borderColor: 'var(--color-border)' }}>
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium transition-all ${
                isActive ? 'bg-purple-500/15 text-purple-400' : 't-tertiary'
              }`
            }
          >
